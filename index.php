...
echo "<table class='table' style='font-size: 30px;'>
    ...
    <tbody>
      <tr class='active'>
        <td>Sensor Suhu (°C)</td>
        <td>Sensor Tegangan (V)</td>
        <td>Sensor Arus (A)</td>
		<td>Kecepatan Kipas (Rpm)</td>
      </tr>  
		";
...
while($row = mysqli_fetch_array($result)) {
   ...
   $cur_sent_bool_1 = $row['SENT_BOOL_1'];
	...
	<input type='hidden' name='value' value=$inv_current_bool_1  size='15' >	
  	...
  	<input type= 'submit' name= 'change_but' style=' margin-left: 5%; margin-top: 1%; font-size: 30px; text-align:center; background-color: $color_current_bool_1' value=$text_current_bool_1></form></td>";
	...
	<input type='hidden' name='value' value=$inv_current_bool_2  size='15' >	
  	...
  	<input type= 'submit' name= 'change_but' style=' margin-left: 10%; margin-top: 1%; font-size: 30px; text-align:center; background-color: $color_current_bool_2' value=$text_current_bool_2></form></td>";
...
echo "<table class='table' style='font-size: 30px;'>
	<thead>
		<tr>
		<th>Masukan Frekuensi Motor</th>	
		</tr>
	</thead>
	
    <tbody>
      <tr class='active'>
        <td>Frekuensi dari 0, 40, 45, 50, 55, 60, 65 Hz</td>
      </tr>  
		";
...
while($row = mysqli_fetch_array($result)) {
	...
	echo "<td><form action= update_values.php method= 'post'>
  	<input style='width: 100%;' type='text' name='value' value=$current_text_1  size='50'>
  	<input type='hidden' name='unit' value=$unit_id >
